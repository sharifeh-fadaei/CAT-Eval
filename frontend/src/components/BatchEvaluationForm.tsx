...
const BatchEvaluationForm: React.FC<Props> = ({ apiBase, models }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!datasetRoot.trim()) {
      setStatus('Provide dataset root path (where images live).');
      return;
    }
    ...
    if (!selectedModels.filter(Boolean).length) {
  ...
  return (
    <div className="card">
      ...
      <p>Submit a dataset file plus image root. Outputs are JSON files inside the run bundle.</p>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <label>Dataset root (path where sample images live)</label>
        <input
          type="text"
          value={datasetRoot}
          onChange={(e) => setDatasetRoot(e.target.value)}
          placeholder="/path/to/dataset_root"
        />
